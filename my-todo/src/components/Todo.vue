      ...
      <v-card>
        ...
        <v-form ref="form" lazy-validation @submit.prevent="addTodo">
          ...
          <v-btn icon ripple>
            <v-menu>
              <template v-slot:activator="{ on }">
                <v-icon color="grey lighten-1" v-on="on">mdi-Pencil</v-icon>
              </template>
              <v-date-picker v-model="newTodo.datetime" no-title scrollable>
                ...
                <v-btn flat color="primary" @click="$refs.menu.save(newTodo.datetime)"
                  >OK</v-btn
                >
              </v-date-picker>
            </v-menu>
          ...
          <v-btn icon ripple>
            <v-menu>
              <template v-slot:activator="{ on }">
                <v-icon color="grey lighten-1" v-on="on">mdi-folder</v-icon>
              </template>
              <v-text-field
                solo
                append-icon="mdi-pencil"
